...
export class QuestionService {
  ...
  public async submitVote(questionId: number, votedUsersId: number[], writtenAnswer?: string): Promise<Vote[] | null> {
    try {
      const response = await firstValueFrom(this.httpClient.post<ApiResponse>(`${environment.apiUrl}questions/${questionId}/vote/`, { votedUsers: votedUsersId, writtenAnswer }, { withCredentials: true }));
